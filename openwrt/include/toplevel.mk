...
%_zx: scripts/config/conf prepare-tmpinfo FORCE
	if [ \! -e $(TOPDIR)/zxconfig/$(@:_zx=) ] ; then \
		$(_SINGLE)$(NO_TRACE_MAKE) menuconfig $(PREP_MK); \
	fi
	$(TOPDIR)/scripts/zxconfig.sh $(TOPDIR)/zxconfig $(TOPDIR)/zxconfig/$(@:_zx=) $(TOPDIR)/.config
