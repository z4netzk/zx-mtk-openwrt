...
#ifdef USE_NLWIFI
/* mtkwifi */
LUA_WRAP_INT_OP(nlwifi,channel)
LUA_WRAP_INT_OP(nlwifi,frequency)
LUA_WRAP_INT_OP(nlwifi,frequency_offset)
LUA_WRAP_INT_OP(nlwifi,txpower)
LUA_WRAP_INT_OP(nlwifi,txpower_offset)
LUA_WRAP_INT_OP(nlwifi,bitrate)
LUA_WRAP_INT_OP(nlwifi,signal)
LUA_WRAP_INT_OP(nlwifi,noise)
LUA_WRAP_INT_OP(nlwifi,quality)
LUA_WRAP_INT_OP(nlwifi,quality_max)
LUA_WRAP_STRING_OP(nlwifi,ssid)
LUA_WRAP_STRING_OP(nlwifi,bssid)
LUA_WRAP_STRING_OP(nlwifi,country)
LUA_WRAP_STRING_OP(nlwifi,hardware_name)
LUA_WRAP_STRING_OP(nlwifi,phyname)
LUA_WRAP_STRUCT_OP(nlwifi,mode)
LUA_WRAP_STRUCT_OP(nlwifi,assoclist)
LUA_WRAP_STRUCT_OP(nlwifi,txpwrlist)
LUA_WRAP_STRUCT_OP(nlwifi,scanlist)
LUA_WRAP_STRUCT_OP(nlwifi,freqlist)
LUA_WRAP_STRUCT_OP(nlwifi,countrylist)
LUA_WRAP_STRUCT_OP(nlwifi,hwmodelist)
LUA_WRAP_STRUCT_OP(nlwifi,encryption)
LUA_WRAP_STRUCT_OP(nlwifi,mbssid_support)
LUA_WRAP_STRUCT_OP(nlwifi,hardware_id)
#endif
...
#ifdef USE_NLWIFI
/* nlwifi table */
static const luaL_reg R_nlwifi[] = {
	LUA_REG(nlwifi,channel),
	LUA_REG(nlwifi,frequency),
	LUA_REG(nlwifi,frequency_offset),
	LUA_REG(nlwifi,txpower),
	LUA_REG(nlwifi,txpower_offset),
	LUA_REG(nlwifi,bitrate),
	LUA_REG(nlwifi,signal),
	LUA_REG(nlwifi,noise),
	LUA_REG(nlwifi,quality),
	LUA_REG(nlwifi,quality_max),
	LUA_REG(nlwifi,mode),
	LUA_REG(nlwifi,ssid),
	LUA_REG(nlwifi,bssid),
	LUA_REG(nlwifi,country),
	LUA_REG(nlwifi,assoclist),
	LUA_REG(nlwifi,txpwrlist),
	LUA_REG(nlwifi,scanlist),
	LUA_REG(nlwifi,freqlist),
	LUA_REG(nlwifi,countrylist),
	LUA_REG(nlwifi,hwmodelist),
	LUA_REG(nlwifi,encryption),
	LUA_REG(nlwifi,mbssid_support),
	LUA_REG(nlwifi,hardware_id),
	LUA_REG(nlwifi,hardware_name),
	LUA_REG(nlwifi,phyname),
	{ NULL, NULL }
};
#endif
...
#ifdef USE_NLWIFI
	luaL_newmetatable(L, IWINFO_NLWIFI_META);
	luaL_register(L, NULL, R_common);
	luaL_register(L, NULL, R_nlwifi);
	lua_pushvalue(L, -1);
	lua_setfield(L, -2, "__index");
	lua_setfield(L, -2, "nlwifi");
#endif
