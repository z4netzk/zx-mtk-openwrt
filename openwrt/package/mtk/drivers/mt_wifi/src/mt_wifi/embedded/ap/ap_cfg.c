...
COUNTRY_CODE_TO_COUNTRY_REGION *RTMPGetCountryByIsoName(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	INT index = 0;

	for (index = 0; index < NUM_OF_COUNTRIES; index++)
	{
		if (strncmp((RTMP_STRING *) allCountry[index].IsoName, arg, strlen(arg)) == 0)
			return &allCountry[index];
	}
	return NULL;
}
