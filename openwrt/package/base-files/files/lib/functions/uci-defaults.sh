...
ucidef_set_interface_lan() {
	ucidef_set_interface "lan" device "$1" type "bridge" protocol "${2:-static}"
}
...
ucidef_set_lede_system() {
	...
	local w_mac=$6
	local s_country=$7
	local s_manufacturer=$8
	local s_url=$9

	uci -q get lede.system && return 0

	uci batch <<EOF
set lede.system='system'
set lede.system.name='$s_name'
set lede.system.type='$s_type'
set lede.system.def_type='$s_type'
set lede.system.wanport='$s_wan'
set lede.system.cpuport='$s_cpu'
set lede.system.mac='$s_mac'
set lede.system.wanmac='$w_mac'
set lede.system.country='$s_country'
set lede.system.manufacturer='$s_manufacturer'
set lede.system.url='$s_url'
EOF

}

ucidef_set_lede_password() {
	local s_user=$1
	local s_pwd=$2
	local def_pwd_file="/tmp/lede_pwd_info"

	[ -z "$s_pwd" ] && s_pwd="admin"

	uci -q get lede.admin && return 0

	uci batch <<EOF
set lede.admin='admin'
set lede.admin.user='$s_user'
set lede.admin.password='$s_pwd'
set lede.admin.def_password='$s_pwd'
EOF

	if [ "$s_pwd"x != "admin"x ]; then
	cat <<EOF >$def_pwd_file
$s_pwd
$s_pwd
EOF

		passwd root <$def_pwd_file
		rm -rf $def_pwd_file
	fi
}
...
set network.lte$index.metric='$metric'
set network.lte$index.disabled='$disabled'
EOF
...
set network.lte${index}6.metric='$metric'
set network.lte${index}6.disabled='$disabled'
EOF
...
ucidef_set_interface_wan6p() {
	local ifname=$1
	local proto=$2
	local dis=$3

	uci -q get network.wan6p && return 0

	uci batch <<EOF
set network.wan6p='interface'
set network.wan6p.ifname='$ifname'
set network.wan6p.def_ifname='$ifname'
set network.wan6p.proto='$proto'
set network.wan6p.disabled='$dis'
EOF

}

ucidef_set_lede_guest() {
	local guest_ip=$1
	local guest_netmask=$2
	local guest_ifname=$3
	local guest_disable=$4
	local guest_mac=$5

	uci -q get network.guest && return 0

	uci batch <<EOF
set network.guest='interface'
set network.guest.type='bridge'
set network.guest.proto='static'
set network.guest.ifname='$guest_ifname'
set network.guest.def_ifname='$guest_ifname'
set network.guest.ipaddr='$guest_ip'
set network.guest.netmask='$guest_netmask'
set network.guest.disabled='$guest_disable'
set network.guest.macaddr='$guest_mac'
EOF
}